    ...
    <Sidebar className={`${collapsed ? "w-16" : "w-64"} bg-sidebar border-r border-primary/20`}>
      <SidebarHeader className="p-4">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              <h2 className="font-bold font-orbitron text-primary">Strike A Light</h2>
              ...
              {menuItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <a 
                      href={item.url}
                      className="flex items-center gap-3 px-3 py-2 transition-all duration-200 rounded-lg hover:bg-sidebar-accent hover:text-primary group"
                    >
              ...
              <SidebarMenuItem>
                <SidebarMenuButton asChild>
                  <Button
                    onClick={logout}
                    variant="ghost"
                    className="justify-start w-full gap-3 text-destructive hover:text-destructive hover:bg-destructive/10"
                  >
