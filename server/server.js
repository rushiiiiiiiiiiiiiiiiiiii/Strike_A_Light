...
app.post("/vouchers", (req, res) => {
  ...
  if (!["student", "individual"].includes(userType)) {
    return res.status(400).json({ error: "userType must be 'student' or 'individual'" });
  }

  // Parse numbers safely
  const assignedPlaysNum = parseInt(assignedPlays, 10);
  if (isNaN(assignedPlaysNum) || assignedPlaysNum <= 0) {
    return res.status(400).json({ error: "assignedPlays must be a positive number" });
  }
  ...
  const sql = `
    INSERT INTO vouchers 
      (token, user_id, user_type, institution_id, name, assigned_plays, used_plays, amount_paid, status, expires_at, created_at, updated_at)
  ...
  console.log("Inserting voucher:", values); // ✅ log to debug
  ...
  db.query(sql, values, (err, result) => {
    ...
    res.json({
      ...
      name,
      assignedPlays: assignedPlaysNum,
      usedPlays: 0,
